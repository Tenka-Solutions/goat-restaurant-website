...
import { eventInquiryEmailText, validateEventInquiry } from "@/lib/event-inquiry";
...
export async function POST(request: NextRequest) {
  ...
  const result = validateEventInquiry(input);
  ...
  try {
    const response = await fetch("https://api.resend.com/emails", {
      ...
      body: JSON.stringify({
        ...
        subject: `[G.O.A.T. Event Inquiry] ${result.data.name} - ${result.data.preferredDate}`,
        text: eventInquiryEmailText(result.data),
      }),
