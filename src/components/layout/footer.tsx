...
export function Footer({ locale }: { locale: Locale }) {
  ...
  const mapEmbedUrl = businessDetails.address
    ? `https://www.google.com/maps?q=${encodeURIComponent(businessDetails.address)}&output=embed`
    : null;
  const links = [
  ...
  return (
    <footer className="border-t border-ivory/10 bg-ink px-5 pb-8 pt-14 text-ivory sm:px-8 lg:px-12">
      <div className="mx-auto grid max-w-7xl gap-12 md:grid-cols-2 lg:grid-cols-[1.25fr_.65fr_.65fr_1.15fr] lg:gap-8 xl:gap-12">
        <div>
        ...
        <div>
          <p className="mb-4 text-[0.65rem] font-semibold uppercase tracking-[0.2em] text-gold">{copy.footer.location}</p>
          {businessDetails.address && businessDetails.mapUrl && (
            <a href={businessDetails.mapUrl} target="_blank" rel="noopener noreferrer" className="block max-w-[18rem] text-sm leading-6 text-ivory/65 hover:text-sky">
              {businessDetails.address}
            </a>
          )}
          {mapEmbedUrl && (
            <div className="mt-4 h-40 w-full max-w-[18rem] overflow-hidden border border-ivory/10 bg-ink-soft">
              <iframe
                src={mapEmbedUrl}
                title={copy.footer.mapTitle}
