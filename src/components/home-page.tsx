...
export async function HomePage({ locale }: { locale: Locale }) {
  ...
  return <main id="content">
    ...
    <section id="visit" className="bg-ink-soft px-5 py-24 sm:px-8 lg:px-12"><div className="mx-auto max-w-7xl"><SectionHeading eyebrow={copy.visit.eyebrow} title={copy.visit.title} dark /><div className="mt-12 grid gap-5 md:grid-cols-3">{[[copy.visit.address, businessDetails.address], [copy.visit.hours, businessDetails.openingHours.join(" · ")], [copy.visit.contact, businessDetails.phone || businessDetails.email]].map(([label, value]) => <article key={label} className="border-t border-gold/50 pt-5"><h3 className="text-xs font-bold uppercase tracking-[.2em] text-gold">{label}</h3><p className="mt-4 text-sm leading-6 text-ivory/65">{value || copy.visit.pending}</p></article>)}</div></div></section>
