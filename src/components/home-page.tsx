...
import { getHomePageImages } from "@/sanity/lib/client";
import { getVisiblePromotions } from "@/lib/promotions";
import type { Locale } from "@/types/site";

export async function HomePage({ locale }: { locale: Locale }) {
  const copy = getDictionary(locale);
  const [promotionEvents, homePageImages] = await Promise.all([getPromotionEvents(), getHomePageImages()]);
  const promotions = getVisiblePromotions(promotionEvents).slice(0, 3);
  return <main id="content">
    <LocalizedHero locale={locale} image={homePageImages.hero} />
    ...
    <section className="bg-ink-soft px-5 py-24 sm:px-8 lg:px-12"><div className="mx-auto max-w-7xl"><SectionHeading eyebrow={copy.flavours.eyebrow} title={copy.flavours.title} body={copy.flavours.body} dark /><div className="mt-12 grid gap-4 sm:grid-cols-2 lg:grid-cols-4">{copy.flavours.items.map((title, index) => <article key={title} className="group relative min-h-80 overflow-hidden bg-ink"><Image src={homePageImages.flavours[index].src} alt="" fill sizes="(max-width: 640px) 100vw, 25vw" className="object-cover transition-transform duration-700 group-hover:scale-105" /><div className="absolute inset-0 bg-gradient-to-t from-ink via-transparent" /><h3 className="absolute bottom-6 left-6 font-display text-4xl text-ivory">{title}</h3></article>)}</div></div></section>
    <section id="experience" className="grid bg-ivory text-ink lg:grid-cols-2"><div className="relative min-h-96 lg:min-h-[38rem]"><Image src={homePageImages.experience.src} alt={homePageImages.experience.alt[locale]} fill sizes="(max-width:1024px) 100vw, 50vw" className="object-cover" /></div><div className="flex items-center px-5 py-20 sm:px-10 lg:px-16"><SectionHeading eyebrow={copy.story.eyebrow} title={copy.story.title} body={copy.story.body} /></div></section>
