...
import type { PublicMenuCategory } from "@/sanity/menu-source";
import type { Locale } from "@/types/site";

export function MenuPage({ locale, menuCategories }: { locale: Locale; menuCategories: PublicMenuCategory[] }) {
  ...
  return <main id="content" className="bg-ink text-ivory">
    ...
    <section id="restaurant" className="scroll-mt-20 px-5 py-16 sm:px-8 lg:px-12"><div className="mx-auto max-w-7xl"><nav aria-label={copy.index} className="border-y border-ivory/10 py-5"><p className="mb-3 text-[.62rem] font-bold uppercase tracking-[.2em] text-ivory/40">{copy.index}</p><div className="flex flex-wrap gap-x-5 gap-y-3">{menuCategories.map((category) => <a className="text-xs font-semibold text-sky-light hover:text-sky" href={`#${category.id}`} key={category.id}>{category.title[locale]}</a>)}</div></nav><div className="mt-8 grid gap-x-16 lg:grid-cols-2">{menuCategories.map((category) => <section id={category.id} key={category.id} className="scroll-mt-24 border-b border-ivory/10 py-12"><h2 className="font-display text-5xl tracking-wide text-gold">{category.title[locale]}</h2>{category.description && <p className="mt-2 text-sm leading-6 text-ivory/50">{category.description[locale]}</p>}<ul className="mt-7 space-y-7">{category.items.map((entry) => <li key={entry.id}><div className="flex items-start gap-4"><h3 className="min-w-0 flex-1 text-sm font-bold uppercase tracking-[.06em]">{entry.name[locale]}</h3>{entry.price && <span className="shrink-0 font-semibold tabular-nums text-sky-light">{entry.price[locale]}</span>}</div>{entry.description && <p className="mt-2 max-w-xl text-sm leading-6 text-ivory/55">{entry.description[locale]}</p>}</li>)}</ul></section>)}</div></div></section>
